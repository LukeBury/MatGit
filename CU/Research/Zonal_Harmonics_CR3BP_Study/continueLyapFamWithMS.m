...
step_by_x_withYFixed  = false;
step_by_Tp_withYFixed = true;
...
PO_0 = [1.0255633843799135,-0.0000000000000000,0.0000000000000000,0.0000000001195671,-0.0429244710155753,0.0000000000000000,3.5034873291746607];
...
% -------------------------------------------------
%%% Create spectrum of target values
% -------------------------------------------------
n_Solutions = 50;
if step_by_x_withYFixed
    x0s = linspace(PO_0(1), 1-prms.u+1/rNorm,n_Solutions);
elseif step_by_Tp_withYFixed
    Tps = linspace(PO_0(7), 3.0765043887839072,n_Solutions);
end
...
for solutionIndex = 1:n_Solutions
    ...
    if step_by_x_withYFixed
        x0 = x0s(solutionIndex);
    elseif step_by_Tp_withYFixed
        Tp0 = Tps(solutionIndex);
    end
...
% %      
    ...
    while (c_norm > error_tol) && (iter < maxIter)
        ...
        if step_by_x_withYFixed
            [DF, constraints_vec] = multShooter_LyapPO_stateContinuity(N_nodes, x0, F_vec, @Int_CR3BnSTM, options, prms);
        elseif step_by_Tp_withYFixed
            [DF, constraints_vec] = multShooter_stateContinuity_yzxdzdTpFixed(N_nodes, F_vec, @Int_CR3BnSTM, options, prms, Tp0);
        end
    ...
    if c_norm > error_tol
        ...
        elseif step_by_Tp_withYFixed
            POs(solutionIndex,:) = [F_vec(1); 0; 0; 0; F_vec(2); 0; Tp0];
        end
...
if savePOs 
    ...
    fileAlreadyExists = 1;
    fileVersion       = 0;
    famName_new       = [famName,'.CR3BP.L2_Lyapunov_new'];
