...
% is it the case that the null space increases dimension near a tangent bifurcation but
...
%%% Run Switches
% ========================================================================
%%% Center the integration at the secondary body instead of barycenter?
use_scr_integration = true;
% save_as_scr         = true;

%%% Which shooter to run? true or false
run_y0Fixed                         = false;  % PSEUDO-ARCLENGTH

run_y0xd0Fixed                      = false; % PSEUDO-ARCLENGTH
...
modelName = 'CR3BP'
% modelName = 'CR3BP_J2pJ4pJ6pJ2s'
% modelName = 'CR3BP_J2pJ4pJ6pJ2s_C22s'
...
% -------------------------------------------------
%%% Maxum number of POs to find in family, regardless of step size
n_POs_max = 5000;
...
iterLimitStepSizeReward = 1.021; % medium
...
%%% Certain penalty flags will gradually lower the continuation step size for PAC,
...
%%% Set a maximum step size for PAC
ds_PO_maximumValue = 1e-2;
ds_PO_maximumValue = 2e-3;
% ds_PO_maximumValue = 1.5e-3;
...
% ds_PO = 8e-4; % Good starting guess
% ds_PO = 5e-4; % Good starting guess
% ds_PO = 3e-4; % Good starting guess
...
%     Tp_range = linspace(myPO_ICs(7), 2.1312325938455454, 20); 
    
%     Tp_range = myPO_ICs(7):1e-7:(myPO_ICs(7)+6);  
    Tp_range = myPO_ICs(7):-5e-4:(max([myPO_ICs(7)-6, 0])); 
...
elseif isequal(modelName, 'CR3BP_J2pJ4pJ6pJ2s')
    prms.J2p = primary.J2;
    prms.J4p = primary.J4;
    prms.J6p = primary.J6;
    ...
    %%% Set integrator handle
    integratorHandle = @Int_CR3BnSTM_J2pJ4pJ6pJ2s;

    if use_scr_integration
        warning('Haven''t made this integrator')
        return
    end

elseif isequal(modelName, 'CR3BP_J2pJ4pJ6pJ2s_C22s')
    prms.J2p  = primary.J2;
    prms.J4p  = primary.J4;
    prms.J6p  = primary.J6;
    prms.J2s  = secondary.J2;
...
if use_scr_integration
    myPO_ICs(1) = myPO_ICs(1) - (1-prms.u);
...
if correct_initial_guess_to_y0equals0
    ...
    if myPO_ICs(2) ~= 0
        ...
        [Tfix_fwd, Xfix_fwd, ~, ~, ~] = ode113(integratorHandle, [0, myPO_ICs(7)], [myPO_ICs(1:6); reshape(eye(6),36,1)], options_yEquals0_terminal, prms);
        [Tfix_bkwd, Xfix_bkwd, ~, ~, ~] = ode113(integratorHandle, [myPO_ICs(7), 0], [myPO_ICs(1:6); reshape(eye(6),36,1)], options_yEquals0_terminal, prms);
        
        %%% Use this to choose a y=0 crossing. Ideally looking for one
        %%% where xd and zd are also 0
        if 1+1==1
            [Tfix_fwd_non, Xfix_fwd_non, tEv_fwd_non, xEv_fwd_non, ~] = ode113(integratorHandle, [0, myPO_ICs(7)], [myPO_ICs(1:6); reshape(eye(6),36,1)], options_yEquals0_nonTerminal, prms);
            xEv_fwd_non(:,1:6)
        end
        %%% Times to y=0
    ...
    %%% Plot initial guess
    [T_guess, X_guess] = ode113(integratorHandle, [0, myPO_ICs(7)], [myPO_ICs(1:6); reshape(eye(6),36,1)], options, prms);
    figure; hold all
    title('guess')
    ...
    if run_y0Fixed
        [PO_0, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0Fixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0);
    elseif run_y0xd0Fixed
        [PO_0, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0xd0Fixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0);
    elseif run_y0zd0Fixed
        [PO_0, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0zd0Fixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0);
    elseif run_y0xd0zd0Fixed
        [PO_0, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0xd0zd0Fixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0, 0);
    elseif run_y0TpFixed
        [PO_0, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0TpFixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, PO_0_guess(7));
    elseif run_y0xd0TpFixed
        [PO_0, counter, constraint_error] = correctPO_mS_sC_y0xd0TpFixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0, PO_0_guess(7));
    elseif run_y0xd0zd0TpFixed
        [PO_0, counter, constraint_error] = correctPO_mS_sC_y0xd0zd0TpFixed(PO_0_guess, n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0, 0, PO_0_guess(7));
    end
    
    fprintf('Initial guess has converged\n')

    % [T, X] = ode113(integratorHandle, [0, PO_0(7)], [PO_0(1:6); reshape(eye(6),36,1)], options, prms);
    % [T_corrected, X_corrected] = ode113(integratorHandle, linspace(0, PO_0(7), 10000), [PO_0(1:6); reshape(eye(6),36,1)], options, prms);
    [T_corrected, X_corrected] = ode113(integratorHandle, linspace(0, PO_0(7), 10000), [PO_0(1:6); reshape(eye(6),36,1)], options, prms);
    figure; hold all
    title('corrected')
...
[Tref_n, Xref_n] = ode113(integratorHandle, [0, PO_0(7)], [PO_0(1:6); stm0_colVec], options, prms);
...
%     view(0, 90) % x-y
%     view(90, 0) % y-z
    if sum(Xref_n(:,3) + sum(Xref_n(:,6))) == 0
        view(0, 90) % x-y
    else
        view(0, 0) % x-z
    end
...
if use_scr_integration
    impactFlags(1) = min(rowNorm(Xref_n(:,1:3))) < prms.R2;
else
...
[~, X_nodes] = get_nodes([PO_0(1:6); stm0_colVec], [0, PO_0(7)], n_Nodes+1, integratorHandle, options, prms);
...
%%% Jacobi constant
if use_scr_integration
    jacobiConstants(1) = getJacobiConstant_ZH(POs(1,1:6) + [1-prms.u,0,0,0,0,0], prms);
...
while PO_i <= n_POs_max
    ...
    if run_y0Fixed
        z_comp_old = F_new(2) + F_new(5);
        [F_new, constraint_error, DF, counter] = pseudoArclengthContinuation_multShoot_stateContinuity_y0Fixed(...
            error_tol, ms_iterMax, n_Nodes, F_new, integratorHandle, options, prms, PO_i, stepSize, nullVecDF, ds_PO, 0);
        z_comp_new = F_new(2) + F_new(5);

    elseif run_y0xd0Fixed
        z_comp_old = F_new(2) + F_new(4);
        [F_new, constraint_error, DF, counter] = pAC_mS_sC_y0xd0Fixed(...
            error_tol, ms_iterMax, n_Nodes, F_new, integratorHandle, options, prms, PO_i, stepSize, nullVecDF, ds_PO, 0, 0);
        z_comp_new = F_new(2) + F_new(4);

    elseif run_y0zd0Fixed
        z_comp_old = F_new(2) + 0;
        [F_new, constraint_error, DF, counter] = pAC_mS_sC_y0zd0Fixed(...
            error_tol, ms_iterMax, n_Nodes, F_new, integratorHandle, options, prms, PO_i, stepSize, nullVecDF, ds_PO, 0, 0);
        z_comp_new = F_new(2) + 0;

    elseif run_y0xd0zd0Fixed
        z_comp_old = F_new(2) + 0;
        [F_new, constraint_error, DF, counter] = pAC_mS_sC_y0xd0zd0Fixed(...
            error_tol, ms_iterMax, n_Nodes, F_new, integratorHandle, options, prms, PO_i, stepSize, nullVecDF, ds_PO, 0, 0, 0);
        z_comp_new = F_new(2) + 0;
    ...
    elseif run_y0TpFixed
        ...
        [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0TpFixed(...
            [F_new(1); 0; F_new(2:5)], n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, Tp_i);
    elseif run_y0xd0TpFixed
        Tp_i = Tp_range(PO_i);
        PO_newGuess = POs(PO_i,:)';
        [PO_new, counter, constraint_error] = correctPO_mS_sC_y0xd0TpFixed(...
            PO_newGuess(1:6), n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0, Tp_i);
        
    elseif run_y0xd0zd0TpFixed
        Tp_i = Tp_range(PO_i);
        PO_newGuess = POs(PO_i,:)';
        [PO_new, counter, constraint_error] = correctPO_mS_sC_y0xd0zd0TpFixed(...
            PO_newGuess(1:6), n_Nodes, integratorHandle, options, prms, error_tol, ms_iterMax, stepSize, 0, 0, 0, Tp_i);
    end
    

    %%% If the z-component collapsed in PAC, lower the step size and try again
    if (~run_y0TpFixed && ~run_y0xd0zd0TpFixed && ~run_y0xd0TpFixed)
        if abs(z_comp_old / z_comp_new) > 1e5
            warning('Z-Component Collapsed ... lowering ds_PO and trying again')
...
%         if (~run_y0TpFixed && ~run_y0xd0zd0TpFixed)
        ...
        % -------------------------------------------------
        %%% Check for family continuity (no jumps to other equilibria or loss of 3rd dimension)
        % -------------------------------------------------
...
%                 end
        ...
        %%% Store converged F
        if run_y0TpFixed
            [~, X_nodes] = get_nodes([PO_new(1:6); stm0_colVec], [0, PO_new(7)], n_Nodes+1, integratorHandle, options, prms);
            X_nodes = X_nodes(1:n_Nodes,1:6);
            F_new   = reshape(X_nodes',6*n_Nodes,1);
            F_new = [F_new(1); F_new(3:end)];
        end
        ...
        %%% Integrate new PO
        if run_y0Fixed
            [~, X_new] = ode113(integratorHandle, [0, F_new(end)], [F_new(1); 0; F_new(2:5); stm0_colVec], options, prms);
        elseif run_y0xd0Fixed
            [~, X_new] = ode113(integratorHandle, [0, F_new(end)], [F_new(1); 0; F_new(2); 0; F_new(3:4); stm0_colVec], options, prms);
        elseif run_y0zd0Fixed
            [~, X_new] = ode113(integratorHandle, [0, F_new(end)], [F_new(1); 0; F_new(2:4); 0; stm0_colVec], options, prms);
        elseif run_y0xd0zd0Fixed
            [~, X_new] = ode113(integratorHandle, [0, F_new(end)], [F_new(1); 0; F_new(2); 0; F_new(3); 0; stm0_colVec], options, prms);
        elseif run_y0TpFixed
            [~, X_new] = ode113(integratorHandle, [0, Tp_i], [PO_new(1:6); stm0_colVec], options, prms);
        elseif run_y0xd0TpFixed
            [~, X_new] = ode113(integratorHandle, [0, Tp_i], [PO_new(1:6); stm0_colVec], options, prms);
        elseif run_y0xd0zd0TpFixed
            [~, X_new] = ode113(integratorHandle, [0, Tp_i], [PO_new(1:6); stm0_colVec], options, prms);
        ...
        if use_scr_integration
            impactFlags(PO_i+1) = min(rowNorm(X_new(:,1:3))) < prms.R2;
        else
        ...
        if use_scr_integration 
            %%% Energy of new PO
            jacobiConstants(PO_i+1)        = getJacobiConstant_ZH(X_new(1,1:6) + [1-prms.u,0,0,0,0,0], prms);
            
            %%% Altitude
            rough_altitudes = rowNorm(X_new(:,1:3)) - prms.R2;
        else
            %%% Energy of new PO
            jacobiConstants(PO_i+1)        = getJacobiConstant_ZH(X_new(1,1:6), prms);
            
            %%% Altitude
            rough_altitudes = rowNorm(X_new(:,1:3) - [1-prms.u, 0, 0]) - prms.R2;
        end
        %%% Energy of new PO
        landingVelocities_mps(PO_i+1)  = JC_2_approxLandingVelocity(jacobiConstants(PO_i+1), prms, vNorm);

        %%% Altitude
        rough_min_alt = min(rough_altitudes)*rNorm;
        ...
        if counter <= iterLimitForStepSizeIncrease
            ...
            if ds_PO > ds_PO_maximumValue
                ds_PO = ds_PO_maximumValue;
            end
...
%         if run_y0TpFixed || run_y0xd0zd0TpFixed
        if run_y0TpFixed || run_y0xd0zd0TpFixed || run_y0xd0TpFixed
            fprintf('PO %1d ... iter: %1.3d ... prop. error = %1.2e ... min. altitude ~ %1.1fkm ... time: %1.1f sec\n',...
                PO_i, counter, actualErrors(PO_i+1), rough_min_alt, toc(ticWhole))
        else
            fprintf('PO %1d ... iter: %1.3d ... ds_PO = %1.2e ... prop. error = %1.2e ... min. altitude ~ %1.1fkm ... time: %1.1f sec\n',...
                PO_i, counter, ds_PO, actualErrors(PO_i+1), rough_min_alt, toc(ticWhole))
        end
