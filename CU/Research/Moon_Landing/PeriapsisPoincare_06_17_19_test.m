...
% dJC_Lp = 0.000053; % J0 = JC_Lp - dJC_Lp
% for dJC_Lp = [1.3, 2.3, 3.3, 4.3, 5.3]* 1.0e-5 %%% Looping whole thing over JCs
% clear impactTraj binData
...
if plot_referenceTrajectory == 1
    figure
    plot3(X_BCR_n_ref(:,1),X_BCR_n_ref(:,2),X_BCR_n_ref(:,3))
...
%     PlotBoi3('x$_n$','y$_n$','z$_n$',16,'Latex')    
    ...
    for jj = 1:length(binData(kk).JC_postBurn)
        [C2,h2] = contour(X,Y,Z,[binData(kk).JC_postBurn(jj), binData(kk).JC_postBurn(jj)],'color',binData(kk).bin_col,'linewidth',1);
    end
...
%                         'markeredgecolor',colors.std.black,'markerfacecolor',binData(kk).bin_col,'linewidth',2)
    ...
    xlim([-180 180])
    ylim([min(vertcat(binData(:).JC_postBurn))*.999 max(vertcat(binData(:).JC_postBurn))*1.001])
    ...
    xlim([-180 180])
    ylim([min(vertcat(binData(:).impactSpeeds))*.999 max(vertcat(binData(:).impactSpeeds))*1.001].*(vNorm*1000))
    PlotBoi2('Longitude, $^\circ$','Impact Speed, $m/s$',12,'Latex')

    %%% If there was an impact
    if isempty(binData(kk).X_eventImpact) == 0
        %%% Plotting impact longitude vs speed
        plot(binData(kk).impactLatLons(:,2),binData(kk).impactSpeeds.*(vNorm*1000),'.','markersize',markerSize,...
                        'color',binData(kk).bin_col,'linewidth',2)

        %%% Plotting anti-primary line (0-deg longitude)
        [antiPrimaryLine] = plot([0 0],[0 max(vertcat(binData(:).impactSpeeds))].*(vNorm*1000*1.1),'color',colors.std.blue,'linewidth',1.5);
    ...
    if isempty(binData(kk).X_eventImpact) == 0
        plot(binData(kk).impactAngles.*(180/pi),binData(kk).impactSpeeds.*(vNorm*1000),'.','markersize',markerSize,...
                        'color',binData(kk).bin_col,'linewidth',2)
    end
...
fprintf('Done: %1.1f min\n',toc(ticWhole)/60)

%%% Saving figure and closing
% figPath = '/Users/lukebury/Documents/MATLAB/CU/Research/Moon_Landing/Figures/';
% figName = sprintf('%sdJC0_%1.1e.png', figPath, dJC_Lp);
% saveas(gcf,figName)
% close all
% end % Looping whole thing over JCs
