...
dJC_vel_mps = 62.5; % Meters per second (56.5 -> 2e-5 on Europa .... 28.2 -> 5e-6)
...
% dV Specifications
% -------------------------------------------------
dVMags_mps = linspace(10,100,5);

for dVMag_metersPerSecond = dVMags_mps %%% Looping whole thing over dV magnitude
...
if saveFigs == 1
    ...
    figName = sprintf('%s%sL%1d_dV_%1.1f.png', figPath, secondary.name(1:3), Lpoint, dVMag_metersPerSecond);
    saveas(gcf,figName)
    close all
end
