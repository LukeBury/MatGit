...
dJC_vel_mps = 56.5; % Meters per second (56.5 -> 2e-5 on Europa .... 28.2 -> 5e-6)
...
% -------------------------------------------------
% Finding y-value of neck at L-point
% -------------------------------------------------
%%% Set values
c = JC_scInitial;
u = secondary.MR;
...
f = @(y) x^2 + y.^2 - c + 2*(1-u)./sqrt((x+u)^2+y.^2) + 2*u./sqrt((x-1+u)^2+y.^2);
...
% -------------------------------------------------
%%% Create vector of starting y values
if isequal(secondary.name,'Europa') == 1
    y0s_n = linspace(0.0012,y_neck,5);
elseif isequal(secondary.name,'Enceladus') == 1
    y0s_n = linspace(0.0012,y_neck-.0001,5);
end

for y0_n = y0s_n %%% Looping whole thing over neck positions
    
%%% Clear previous data
clear impactTraj binData

%%% Set Initial position
r0_n = [L123(Lpoint,1), y0_n, 0];
...
if plot_referenceTrajectory == 1
    ...
    [Cref,href] = contour(X,Y,Z,[JC_scInitial, JC_scInitial],'color',colors.std.black,'linewidth',3);
    view(0,90)
    return
end
...
%         end
        [C2,h2] = contour(X,Y,Z,[currentContour, currentContour],'color',binData(kk).bin_col,'linewidth',3);
    end
...
if saveFigs == 1
    ...
    figName = sprintf('%s%sL%1d_Neck_%1.1e.png', figPath, secondary.name(1:3), Lpoint, r0_n(2));
    saveas(gcf,figName)
    close all
end
