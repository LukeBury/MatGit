...
if testCaseOn == 0
    ...
    elseif on_zonalHarmonics == 1
        %%% All impact data
        filename_allTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_ZH_data3.txt',...
            computerTag,secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_ZH_land3.txt',...
            computerTag,secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Log file
        filename_runData = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_ZH_log3.txt',...
            computerTag,secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
    end
elseif testCaseOn == 1
    if on_zonalHarmonics == 0
    ...
    elseif on_zonalHarmonics == 1
        %%% All impact data
        filename_allTraj     = fullfile(savepath,'testFile_ZH_data.txt');
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath,'testFile_ZH_land.txt');
        %%% Log file
        filename_runData     = fullfile(savepath,'testFile_ZH_log.txt');
    end
end
