%%% Information
% This script is a flavor of the grid search designed to search with a low
% resolution, find a decent number of impact trajectories (of all impact
% angles), and write the initial conditions to a file. The goal for now is
% to be able to take those trajectories and look at their angular momentums
% to see try and understand the low-energy latitude boundaries
% ========================================================================
clear
clc
close all
...
if testCaseOn == 0
    if on_J21 == 0
        %%% All impact data
        filename_allTraj = fullfile(savepath, sprintf('impactX0s_iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_data.txt',...
            secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Log file
        filename_runData = fullfile(savepath, sprintf('impactX0s_iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_log.txt',...
            secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
    elseif on_J21 == 1
        %%% All impact data
        filename_allTraj = fullfile(savepath, sprintf('impactX0s_iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_J21_data.txt',...
            csecondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Log file
        filename_runData = fullfile(savepath, sprintf('impactX0s_iGS_%sL%1.0f_%1.0fmps_%1.0fkm_%1.0fv0s_J21_log.txt',...
            secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
    end
elseif testCaseOn == 1
    if on_J21 == 0
