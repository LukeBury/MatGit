...
dvLp_mps = 250; % Meters per second

SRs = linspace(0.014,0.190,7);
%%% SRS of a few important moons of the solar system
% moon      - 0.014
% callisto  - 0.024
% titan     - 0.025
% oberon    - 0.029
...
if on_SR_scaling == 1
    if on_J21 == 1
        warning('This is gonna start some shit')
        return
    end
...
if testCaseOn == 0
    if on_J21 == 0
        %%% All impact data
        filename_allTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_SR%1.2f_%1.0fmps_%1.0fkm_%1.0fv0s_data.txt',...
            computerTag,secondary.name(1:3),Lpoint,SR,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_SR%1.2f_%1.0fmps_%1.0fkm_%1.0fv0s_land.txt',...
            computerTag,secondary.name(1:3),Lpoint,SR,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Log file
        filename_runData = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_SR%1.2f_%1.0fmps_%1.0fkm_%1.0fv0s_log.txt',...
            computerTag,secondary.name(1:3),Lpoint,SR,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
    elseif on_J21 == 1
        %%% All impact data
        filename_allTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_SR%1.2f_%1.0fmps_%1.0fkm_%1.0fv0s_J21_data.txt',...
            computerTag,secondary.name(1:3),Lpoint,SR,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_SR%1.2f_%1.0fmps_%1.0fkm_%1.0fv0s_J21_land.txt',...
            computerTag,secondary.name(1:3),Lpoint,SR,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Log file
        filename_runData = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_SR%1.2f_%1.0fmps_%1.0fkm_%1.0fv0s_J21_log.txt',...
            computerTag,secondary.name(1:3),Lpoint,SR,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
    end
elseif testCaseOn == 1
    if on_J21 == 0
        %%% All impact data
        filename_allTraj     = fullfile(savepath,sprintf('testFile_SR%1.2f_data.txt',SR));
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath,sprintf('testFile_SR%1.2f_land.txt',SR));
        %%% Log file
        filename_runData     = fullfile(savepath,sprintf('testFile_SR%1.2f_log.txt',SR));
    elseif on_J21 == 1
        %%% All impact data
        filename_allTraj     = fullfile(savepath,sprintf('testFile_SR%1.2f_J21_data.txt',SR));
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath,sprintf('testFile_SR%1.2f_J21_land.txt',SR));
        %%% Log file
        filename_runData     = fullfile(savepath,sprintf('testFile_SR%1.2f_J21_log.txt',SR));
    end
end
...
fprintf(f_runData,'SizeRatio:%1.6f\n',SR);
