%%% Like iGS_main, but accounts for J2 in the dynamics of the trajectories
%%% - does NOT account for J2 when creating the neck and looking at initial
%%% JC values. Meant to shed light on errors to expect from model
%%% innacuracies
...
%     elseif on_J21 == 1
        %%% All impact data
        filename_allTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_J21Comp_%1.0fmps_%1.0fkm_%1.0fv0s_data.txt',...
            computerTag,secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_J21Comp_%1.0fmps_%1.0fkm_%1.0fv0s_land.txt',...
            computerTag,secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
        %%% Log file
        filename_runData = fullfile(savepath, sprintf('%s.iGS_%sL%1.0f_J21Comp_%1.0fmps_%1.0fkm_%1.0fv0s_log.txt',...
            computerTag,secondary.name(1:3),Lpoint,dvLp_mps,r0GridSpacing_km,n_v0s_per_r0));
%     end
elseif testCaseOn == 1
%     if on_J21 == 0
...
%     elseif on_J21 == 1
        %%% All impact data
        filename_allTraj     = fullfile(savepath,'testFile_J21Comp_data.txt');
        %%% Low-impact-angle data
        filename_landingTraj = fullfile(savepath,'testFile_J21Comp_land.txt');
        %%% Log file
        filename_runData     = fullfile(savepath,'testFile_J21Comp_log.txt');
%     end
end
