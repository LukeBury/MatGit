...
%%% Making Smarter Data
% ------------------------------------------------------------------------
%%% Creating matrix of initial station parameters (position; velocity)
statPrms0_ECI = zeros(6,3);
statPrms0_ECI(:,1) = [rStat1_ECEF(1,:)'; vStat1_ECI_0(1,:)'];
statPrms0_ECI(:,2) = [rStat2_ECEF(1,:)'; vStat2_ECI_0(1,:)'];
statPrms0_ECI(:,3) = [rStat3_ECEF(1,:)'; vStat3_ECI_0(1,:)'];

%%% Creating matrix of all range & rangeRate measurements w/ station #
measurements = zeros(length(Times),3);
for k = 1:length(Times)
    if range1_LEO_n(k) ~= 0 && range2_LEO_n(k) == 0 && range3_LEO_n(k) == 0 % Station 1
        measurements(k,1) = 1;
    elseif range1_LEO_n(k) == 0 && range2_LEO_n(k) ~= 0 && range3_LEO_n(k) == 0 % Station 2
        measurements(k,1) = 2;
    elseif range1_LEO_n(k) == 0 && range2_LEO_n(k) == 0 && range3_LEO_n(k) ~= 0 % Station 3
        measurements(k,1) = 3;
    end
    measurements(k,2) = noisyRangeMeas(k);
    measurements(k,3) = noisyRangeRateMeas(k);
end
...
% 
% %%% Reshaping ref states and STMs
% refStates_Batch = zeros(length(rT),6);
% refSTMs_Batch = zeros(6,6,length(rT));
% for k = 1:length(rT)
%     %%% Reference States
...
for k = 1:length(rT)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if bl > 1
%     %%% Propagate new reference
%     refStates_Batch(1,:) = refStates_Batch(1,1:6) + xHat0_Batch';
%     xHats_Batch(1,:) = xHat0_Batch';
%     for k = 2:length(Times)
%         xHats_Batch(k,:) = (refSTMs_Batch(:,:,k)*xHat0_Batch)';
%         refStates_Batch(k,:) = refStates_Batch(k,1:6) + xHats_Batch(k,:);
%     end
% end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1:length(Times) % Inner Batch Loop
    ...
    if noisyRangeMeas(k) ~= 0 % If we have a measurement at this time step
        ...
        statNum = measurements(k,1);
        th = wE(3) * (Times(k) - Times(1));
        %%% DP/DX
        Htilde(1,1) = DPDxyzCalculator(1, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', th);
        Htilde(1,2) = DPDxyzCalculator(2, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', th);
        Htilde(1,3) = DPDxyzCalculator(3, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', th);
        %%% DdP/DX
        Htilde(2,1) = DdPDxyzCalculator(1, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', refStates_Batch(k,4:6), statPrms0_ECI(4:6,statNum)', th);
        Htilde(2,2) = DdPDxyzCalculator(2, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', refStates_Batch(k,4:6), statPrms0_ECI(4:6,statNum)', th);
        Htilde(2,3) = DdPDxyzCalculator(3, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', refStates_Batch(k,4:6), statPrms0_ECI(4:6,statNum)', th);
        Htilde(2,4) = DdPDvxvyvzCalculator(1, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', th);
        Htilde(2,5) = DdPDvxvyvzCalculator(2, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', th);
        Htilde(2,6) = DdPDvxvyvzCalculator(3, refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', th);
        %%% Prefit Residual
        yi = [Yi(1) - norm(refStates_Batch(k,1:3) - statPrms0_ECI(1:3,statNum));...
            Yi(2) -  rangeRateCalculator(refStates_Batch(k,1:3), statPrms0_ECI(1:3,statNum)', refStates_Batch(k,4:6), statPrms0_ECI(4:6,statNum)', th)];
...
% ------------------------------------------------------------------------
function [ DPDxyz ] = DPDxyzCalculator(n, Rs, Rstat, th)
    Rstat = R3(Rstat, th);
...
% ------------------------------------------------------------------------
function [DdPDXxyz] = DdPDxyzCalculator(n, Rs, Rstat, Vs, Vstat, th)
    Rstat = R3(Rstat, th);
    Vstat = R3(Vstat, th);
    p = norm(Rs-Rstat);
...
% ------------------------------------------------------------------------
function [DdPDvxvyvz] = DdPDvxvyvzCalculator(n, Rs, Rstat, th)
    Rstat = R3(Rstat, th);
...
% ------------------------------------------------------------------------
function [rangeRate] = rangeRateCalculator(Rs, Rstat, Vs, Vstat, th)
    Rstat = R3(Rstat, th);
    Vstat = R3(Vstat, th);
