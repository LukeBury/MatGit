...
for k = 1:length(Times)
   ...
   if k > 1 && noisyRangeMeas(k) == 0 && noisyRangeMeas(k-1) ~= 0
       arcTimes(counter,2) = Times(k);
   end
end

%%% If filters should wait for first measurement
if waitFirstMeas == 1
    mTimeIndex = find(Times==measTimes(2));
...
ekfLimit = 10000; % Measurements before switching to EKF
for k = 2:length(Times)
    if arcCounter > ekfLimit % If we've had at least 20 measurements on this arc
    ...
    if noisyRangeMeas(k) ~= 0 % If there is a measurement
        ...
        %%% Time Update
        if arcCounter < ekfLimit
            xBari = refSTM_EKF*xHats_EKF(k-1,:)';%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%WorkBetterWhenCKF
        end
