...
% ------------------------------------------------------------------------
syms xsE ysE zsE dxsE dysE dzsE xsJ ysJ zsJ uEur uJ
syms xEJ yEJ zEJ dxEJ dyEJ dzEJ
syms xJSun yJSun zJSun dxJSun dyJSun dzJSun
syms xEaSun yEaSun zEaSun dxEaSun dyEaSun dzEaSun
syms xstEa ystEa zstEa dxstEa dystEa dzstEa
...
% ------------------------------------------------------------------------
% Guess for i=35 deg
% r0_BCR_n = [1.0023749; 0; 0]; % km
% v0_BCR_n = [0; 0.08408; 0.05887]; % km/s
...
[Times_n,refStatesSTMs_BCR_n] = ode45(@integrator_FP,time,IC_BCR_n_ref,options,u,rB1_BCR_n,rB2_BCR_n,rad2_n,tNorm,rNorm,vNorm,JD0,uJ,uEur);
...
for k = 2:kEnd
    ...
    if any(data(:,2)==Times(k)) == 1 && measOn == 1 % If there is a measurement (if the data time is in my time vector)
        ...
        %%% Calculating Htilde
        [ Htilde ] = calculateHtilde_EurCI(refStatesSTMS_EurCI(k,1:6), europa_JCI(k,1:6), jupiter_HC(k,1:6), earth_HC(k,1:6), stations_ECI(:,data(m,1),k)');
...
if plotResidualsAndCovariance == 1
    lw = 1.5;
    figure
...
if plotErrorAlone == 1
    figure
    subplot(3,2,1)
    plot(Times./3600,error(:,1),'linewidth',1.5)
    PlotBoi2('','X Error, km',14)
    subplot(3,2,3)
    plot(Times./3600,error(:,2),'linewidth',1.5)
    PlotBoi2('','Y Error, km',14)
    subplot(3,2,5)
    plot(Times./3600,error(:,3),'linewidth',1.5)
    PlotBoi2('Times, hr','Z Error, km',14)
    subplot(3,2,2)
    plot(Times./3600,error(:,4),'linewidth',1.5)
    PlotBoi2('','dX Error, km/s',14)
    subplot(3,2,4)
    plot(Times./3600,error(:,5),'linewidth',1.5)
    PlotBoi2('','dY Error, km/s',14)
    subplot(3,2,6)
    plot(Times./3600,error(:,6),'linewidth',1.5)
    PlotBoi2('Times, hr','dZ Error, km/s',14)
end
figure; hold all
