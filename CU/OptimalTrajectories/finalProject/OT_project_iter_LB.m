...
% X0_n = [L123(2,1), 100/rNorm, 0, 0, 0, 0]';
% Xtgt_n = [1-secondary.MR+secondary.R_n + 900/rNorm, 0, 0, 0, 0, 0]';
...
Xlin_n = Xtgt_n;
% Xlin_n = X0_n - [3000/rNorm, 0, 0, 0, 0, 0]';
dX_f = Xtgt_n - X0_n;
...
if use_nonlinearDynamics == 1
    [time_n, X_BCR_n] = ode113(@Int_CR3Bn_OTproject_nonLin,...
        [time0_n(1:end-1), time0_n(end)-(1e-7)], X0_n, options,Xlin_n, t_0,t_f, dX_f,umax_n, prms);
end
...
for kk = 1:length(time_n)
    [u_i, Pf, P_i, dX_i] = getLinearControls(time_n(kk),X_BCR_n(kk,:)',Xlin_n,t_0,t_f,dX_f,umax_n,prms);
    us(kk,:) = u_i';
...
if use_linearDynamics == 1 && propagateNonlinearTrajectoryWithLinearControls == 1
    ...
    plot3(L123(2,1),0,0,'^','markersize',10,'markeredgecolor',colors.std.black,'markerfacecolor',colors.std.grn)
    PlotBoi3('$x_n$','$y_n$','$z_n$',20,'LaTex')
    plot3(X_BCR_nlDyn_lCtrl_n(:,1),X_BCR_nlDyn_lCtrl_n(:,2),X_BCR_nlDyn_lCtrl_n(:,3),'linewidth',1.5,'color',iterColors(iter,:));
...
plot3(X0_n(1),X0_n(2),X0_n(3),'bo','markersize',10)
plot3(Xtgt_n(1),Xtgt_n(2),Xtgt_n(3),'bx','markersize',10)
pL2 = plot3(L123(2,1),0,0,'^','markersize',10,'markeredgecolor',colors.std.black,'markerfacecolor',colors.std.grn);
PlotBoi3('$x_n$','$y_n$','$z_n$',20,'LaTex')
plotHandles2{iter} = plot3(X_BCR_n(:,1),X_BCR_n(:,2),X_BCR_n(:,3),'linewidth',1.5,'color',iterColors(iter,:));
view(0,90)
axis equal
...
view(0,90)
%%% Creating legend
legend([plotHandles2{:},pL2],[legendNames,'L_2'])
...
function [dX] = Int_CR3Bn_OTproject_Lin(t,X,Xlin_n,t0,tf,dX_f,umax_n,prms)
%%% Inputs:
%          t - normalized time vector
%          X - initial state [6x1]
%          Xlin_n - State being linearized about [6x1]
...
function [u_i, Pf, P_i, dX_i] = getLinearControls(t,X,Xlin_n,t0,tf,dX_f,umax_n,prms)
