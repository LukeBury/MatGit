...
use_nonlinearDynamics = 1;

use_linearDynamics = 0;
...
% t_f = 0.45*pi;
t_f = 0.896763095777190;
t_f_ref = 0.896763095777190;
n_dt = 5000;
time0_n = linspace(0,t_f,n_dt);
time0ref_n = linspace(0,t_f_ref,n_dt);
%%% Making a reference trajectory (ballistically impact lat:20, lon:-90
X0ref_n = [1.0204617015266166, 0.0023309601682839, -0.0034514101589022,...
    -0.0045057561941408, -0.0061368065713195, 0.0008999679154899]';
tol = 2.22045e-14;
...
%%% Europa Landing Site
lats = [25:5:90];
% for lat_tgt_deg = lats
lat_tgt_deg = 55;
lon_tgt_deg = -90;
[rBCR_tgt] = latlon2BCR('secondary', lat_tgt_deg, lon_tgt_deg, secondary.MR, secondary.R_n);
X0_n = X0ref_n;
% Xtgt_n = [0.999974718660022; 0.002185474955331; 0.000795447831475;...
%          -0.119043747282878; -0.057328447219131; -0.026282010885615]; % 
Xtgt_n = [rBCR_tgt; -0.119043747282878; -0.057328447219131; -0.026282010885615]; % 
dX_f = Xtgt_n - Xref_BCR_n(end,:)';
...
if iter == n_iter
fprintf('Targeted Latitude: %1.0d deg\n',lat_tgt_deg)
...
% end % lat_tgt_deg = lats


latitudes = [20:5:90]';
dVs = [73.9;158;258.5;361.7;466.8;573.7;682.6;793.5;906.6;1022.2;1140.1;1260.6;1383.7;1509.3;1637.6];
figure; hold all
plot(latitudes,dVs,'linewidth',1.5,'color',colors.std.blue)
plot(latitudes,dVs,'.','markersize',30,'color',colors.std.blue)
PlotBoi2('Latitude Targeted, $^\circ$','$\Delta V$, $m/s$',15,'LaTex')
