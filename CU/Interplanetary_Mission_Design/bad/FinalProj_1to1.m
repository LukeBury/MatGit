...
tof_resonance_days = 2*days_per_year; % 2 years
tof_resonance_secs = tof_resonance_days*86400; % 2 years
...
for Index_Launch = 1:nPoints
    for Index_VGA_arr = 1:nPoints
        ...
        if (c3s_Launch_p(Index_Launch, Index_VGA_arr) > req_c3_Launch_ub) || (isnan(c3s_Launch_p(Index_Launch, Index_VGA_arr)) == 1)
            continue
        end
...
%                 end
                ...
                if abs(VInfs_VGA_p(Index_VGA_dep,Index_EGA1_arr) - VInfs_VGA_m(Index_Launch,Index_VGA_arr)) > req_VInf_VGA_diff
                    continue
                end
%                 
%                 if (JDi_VGA - JD_Arrivals_VGA_vec(1)) > 80
...
%%% mesh
JD_Departures_EGA2_vec = sort(unique(candidates_prelim_JD_EGA1)) + tof_resonance_days;
...
JD_Arrivals_SOI_vec = linspace(JD_Departures_EGA2_vec(1) + 2.5*days_per_year, JD_2041, nPoints2);
...
for Index_cand_pre = 1:length(candidates_prelim)
    ...
    for Index_EGA2_dep = 1:nPoints2
        for Index_SOI_arr = 1:nPoints2
            ...
            if JD_Departures_EGA2(Index_EGA2_dep, Index_SOI_arr) ~= (candidates_prelim(Index_cand_pre).JD_EGA1 + tof_resonance_days)
                continue
            end
            
            %%% Check VInf_SOI requirement
            if VInfs_SOI_m(Index_EGA2_dep, Index_SOI_arr) > req_vInf_SOI_ub
                continue
            end
            
            %%% Check TOF requirement
            if (candidates_prelim(Index_cand_pre).JD_EGA1 + tof_resonance_days + TOFs_days_leg4(Index_EGA2_dep, Index_SOI_arr) - candidates_prelim(Index_cand_pre).JD_Launch)/days_per_year > req_TOF_yrs_ub
                continue
            end
...
candidates_mid_JD_EGA2           = [candidates_mid.JD_EGA2];
end
...
for Index_cand_mid = 1:length(candidates_mid)
    ...
    [vSc_EGA1_p_jj, vSc_EGA2_m_jj, ~] = lambertSolver(rEarth_EGA1_jj, rEarth_EGA2_jj, tof_resonance_secs, 0, 0, Sun.u); % km/s
    ...
    turningAngles_EGA2_kk = zeros(n_phis,1);
    ...
    for kk = 1:n_phis
        ...
        %%% Calculate turning angle
        turningAngles_EGA2_kk(kk) = acos(dot(vInf_EGA2_m_kk,vInf_EGA2_p_jj)/(VInf_EGA2_m_kk*VInf_EGA2_p_jj));
        
        %%% Calculate EGA2 flyby radius
        rps_EGA2_kk(kk) = (Earth.u/(VInf_EGA2_m_kk*VInf_EGA2_p_jj))*((1/cos((pi-turningAngles_EGA2_kk(kk))/2)) - 1);
    ...
    if smallest_rpMax_jj < (req_flybyRadius_EGA_lb)
...
%     %     legend([p1 p2 p3 p4 p5],'EGA1','EGA2','300 km Altitude','Range of Acceptable \Phi', 'Selected \Phi')
    ...
    if abs(VInf_EGA2_p_jj - VInf_EGA2_m_jj) > req_VInf_EGA2_diff
...
%     n_dt = 1000;
%     time0_leg3 = linspace(JD_EGA1_jj,JD_EGA2_jj,n_dt).*86400;
%     X0_leg3 = [rEarth_EGA1_jj; vInfs_EGA1_p_kk(myPhi_index_jj,:)' + vEarth_EGA1_jj];
%     X0_Earth = [rEarth_EGA1_jj; vEarth_EGA1_jj];
%     [~, X_sc_leg3] = ode113(@Int_2BI, time0_leg3, X0_leg3, options, Sun.u);
%     [~, X_Earth_leg3] = ode113(@Int_2BI, time0_leg3, X0_Earth, options, Sun.u);
%     figure; hold all
%     plot3(X_sc_leg3(:,1),X_sc_leg3(:,2),X_sc_leg3(:,3),'color',colors.std.mag)
%     plot3(X_Earth_leg3(:,1),X_Earth_leg3(:,2),X_Earth_leg3(:,3),'color',colors.std.blue)
%     plot3(X_Earth_leg3(1,1),X_Earth_leg3(1,2),X_Earth_leg3(1,3),'o','color',colors.std.blue)
%     plot3(X_Earth_leg3(end,1),X_Earth_leg3(end,2),X_Earth_leg3(end,3),'x','color',colors.std.blue)
%     axis equal
%     PlotBoi3('X, km','Y, km', 'Z, km', 18, 'LaTex')
    ...
    candidateStruct_final{Index_solution}.VInf_EGA2_diff = abs(VInf_EGA2_m_jj - norm(vInfs_EGA1_p_kk(myPhi_index_jj,:)));
    candidateStruct_final{Index_solution}.phi_rad     = phis_rad(myPhi_index_jj);
    
    candidateStruct_final{Index_solution}.TOF_leg3_sec = tof_resonance_secs;
...
candidates_final_JD_EGA1 = [candidates_final.JD_EGA1];
candidates_final_VInf_EGA2_diff = [candidates_final.VInf_EGA2_diff];


% maxEga1Idx = find(candidates_mid_JD_EGA1 == max(candidates_mid_JD_EGA1));
% minEga1Idx = find(candidates_mid_JD_EGA1 == min(candidates_mid_JD_EGA1));
maxEga1Idx = find(candidates_final_JD_EGA1 == max(candidates_final_JD_EGA1));
minEga1Idx = find(candidates_final_JD_EGA1 == min(candidates_final_JD_EGA1));
