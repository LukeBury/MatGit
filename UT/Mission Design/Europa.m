...
%-------------------------Plotting Parameters------------------------------
plotting=[1];   %Choose preconfigured Plots to be generated
 ...
 movie=[];  %Saves Heliocentric movie file with desired file name (use ' ')
            %Can use mfilename if appropriate (keep empty if not saving file)
 fps=20;    %Adjusts the saved movie frame rate (typically 20-30)          
            
%% -------------------------General Trajectory Parameters--------------------
npl = [0 3 3 3 3 0 1];    %moon's number at which each trajectory occurs 
         %(1=Io, 2=Europa, 3=Ganymede, 4=Callisto)
                    
nn = length(npl);   %(Do not change)number of interplanetary nodes    

event(1:nn)=[17 31 31 31 31 41 24]; % Trajectory Event at each node:
...
TG=7.1;
tof(1:nn-1) = [106 20*TG 17*TG 14*TG 7*TG 6*TG];    %time of flight between nodes (days)
