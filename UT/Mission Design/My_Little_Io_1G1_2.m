...
optimization=4;   %Choose to optimize the initial node's julian date and time's
...
plotting=[9];   %Choose preconfigured Plots to be generated
 ...
 movie=['wtf'];  %Saves Heliocentric movie file with desired file name (use ' ')
            %Can use mfilename if appropriate (keep empty if not saving file)
 fps=20;    %Adjusts the saved movie frame rate (typically 20-30)          
            
%% -------------------------General Trajectory Parameters--------------------
% npl = [0 3 3 3 4 3 4];    %moon's number at which each trajectory occurs 
  npl = [3 0 1];    %moon's number at which each trajectory occurs 
         %(1=Io, 2=Europa, 3=Ganymede, 4=Callisto) 0=spaceburn/midcourse maneuver/Defined-Orbit)
                    
nn = length(npl);   %(Do not change)number of interplanetary nodes    

% event(1:nn)=[17 31 31 31 31 31 31]; % Trajectory Event at each node:
event(1:nn)=[31 41 31]; % Trajectory Event at each node:
...
tof(1:nn-1) = [1.1*TG 2.1*TG];    %time of flight between nodes (days)
grade(1:nn-1) = [0 0];   %directio      n of trajectory between nodes 
                                 %(typically prograde)
                            %Prograde: 0, Retrograde: 1
rev(1:nn-1)= [0 0]; %number of orbital revolutions for trajectory between nodes
...
%+++++++++++++++++++++++++++Single Node Orbit Information++++++++++++++++++
%Jupiter-centric orbital elements
% node=1;      %particular node for orbital elements
% rp = 12.96*Rj;
% ra = 265.1*Rj;
...
ap = (rp+ra)/2; %semimajor axis (km)
ep = 1-rp/ap;    %eccentricity
ip = 3.7;     %inclination (deg)
op = 0;      %right ascension of ascending node (deg)
wp = 0;      %argument of periapsis (deg)
tap = 90;     %true anomaly (deg)
Tp=2*pi*sqrt(ap^3/muj)/86400;
JDp = JD0+Tp/2; %reference juilian date of orbital elements
mu = 0; %gravitational parameter (km^2/s^3)
coenp(1:8,node) = [ap ep ip op wp tap JDp mu]; %classical orbital elements at particular node
...
% % % % % % %Jupiter-centric orbital elements
% % % % % % node=3;      %particular node for orbital elements
% % % % % % rp = 11.09*Rj;
% % % % % % ra = 64.4*Rj;
% % % % % % ap = (rp+ra)/2; %semimajor axis (km)
% % % % % % ep = 1-rp/ap;    %eccentricity
% % % % % % ip = 1.3;     %inclination (deg)
% % % % % % op = 0;      %right ascension of ascending node (deg)
% % % % % % wp = 0;      %argument of periapsis (deg)
% % % % % % tap = 0;     %true anomaly (deg)
% % % % % % Tp=2*pi*sqrt(ap^3/muj)/86400;
...
% % % % % % 
% % % % % % %Jupiter-centric orbital elements
% % % % % % node=4;      %particular node for orbital elements
% % % % % % rp = 10.2*Rj;
% % % % % % ra = 45.9*Rj;
% % % % % % ap = (rp+ra)/2; %semimajor axis (km)
% % % % % % ep = 1-rp/ap;    %eccentricity
% % % % % % ip = 0.8;     %inclination (deg)
% % % % % % op = 0;      %right ascension of ascending node (deg)
% % % % % % wp = 0;      %argument of periapsis (deg)
% % % % % % tap = 0;     %true anomaly (deg)
% % % % % % Tp=2*pi*sqrt(ap^3/muj)/86400;
...
% % % % % % 
% % % % % % %Jupiter-centric orbital elements
% % % % % % node=5;      %particular node for orbital elements
% % % % % % rp = 12.33*Rj;
% % % % % % ra = 55*Rj;
% % % % % % ap = (rp+ra)/2; %semimajor axis (km)
% % % % % % ep = 1-rp/ap;    %eccentricity
% % % % % % ip = 1.61;     %inclination (deg)
% % % % % % op = 0;      %right ascension of ascending node (deg)
% % % % % % wp = 0;      %argument of periapsis (deg)
% % % % % % tap = 0;     %true anomaly (deg)
% % % % % % Tp=2*pi*sqrt(ap^3/muj)/86400;
...
% % % % % % 
% % % % % % %Jupiter-centric orbital elements
% % % % % % node=6;      %particular node for orbital elements
% % % % % % rp = 11.17*Rj;
% % % % % % ra = 37*Rj;
% % % % % % ap = (rp+ra)/2; %semimajor axis (km)
% % % % % % ep = 1-rp/ap;    %eccentricity
% % % % % % ip = 0.2;     %inclination (deg)
% % % % % % op = 0;      %right ascension of ascending node (deg)
% % % % % % wp = 0;      %argument of periapsis (deg)
% % % % % % tap = 0;     %true anomaly (deg)
% % % % % % Tp=2*pi*sqrt(ap^3/muj)/86400;
