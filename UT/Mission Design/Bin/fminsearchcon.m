function [sol, fval, exitflag, output] = ...
        fminsearchcon(funfcn, xo, lb, ub, A, b, Aeq, beq, nonlcon, ...
...
%}
    ...
    if isempty(lb), lb = -inf(size(xo)); end
    if isempty(ub), ub = +inf(size(xo)); end
    ...
    if isscalar(lb) && (N ~= 1), lb = repmat(lb, size(xo)); end
    if isscalar(ub) && (N ~= 1), ub = repmat(ub, size(xo)); end    
    ...
    xo(xo < lb) = lb(xo < lb);  xo(xo > ub) = ub(xo > ub);
        
    % transform initial estimate to its unconstrained counterpart
    xin = xo;                                        % fixed and unconstrained variables   
    xin(lb_only) = sqrt(xo(lb_only) - lb(lb_only));  % lower bounds only   
    xin(ub_only) = sqrt(ub(ub_only) - xo(ub_only));  % upper bounds only
    xin(lb_ub)   = real(asin( 2*(xo(lb_ub) - lb(lb_ub))./ ...
    ...
    function check_input
        ...
        if strcmpi(strictness, 'strict') || strcmpi(strictness, 'superstrict')
            ...
            if ~isempty(Aeq) && any(any(Aeq*xo ~= beq))
    ...
    function [sol, fval, exitflag, output] = optimize_globally(varargin)
        ...
        for ii = 1:popsize
            ...
            [sol_i, fval_i, exitflag_i, output_i] = ...
                fminsearchcon(funfcn, population(:,:,ii), lb, ub, A, b, Aeq, beq, nonlcon,...
        ...
        if evaluations < options.MaxFunEvals
            ...
            [sol, fval, exitflag, output_i] = ...
                fminsearchcon(funfcn, sol, lb, ub, A, b, Aeq, beq, nonlcon,...
                strictness, options, algorithm, varargin{:});
            % adjust output
