...
%% Usage
    ...
    % the global minimum:
    solution = fminsearchcon(rosen, [3 3])    
    
    %%
    % Imposing a lower bound on the variables gives 
    [solution, fval] = fminsearchcon(rosen, [3 3], [2 2], [])
    ...
    solution = fminsearchcon(rosen, [3 3], [], [0.5 0.5])
    ...
    [sol, fval, exitflag, output] = fminsearchcon(rosen, [3 -3], [], [], [], ...
        [], [], [], @nonlcon, [], options);
