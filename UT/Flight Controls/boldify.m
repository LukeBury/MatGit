...
for k=1:length(h)
   ...
   if strcmp(htype,'axes')
      ...
      asize = asize(3:4);
       ...
       set(h(k),'FontSize',FONT);		% 12-pt tick mark labels
	 ...
	 set(h(k),'TickLength',[3/32 2.5*3/32]*scale)	% Gives 3/32" ticks
      end

       set(get(h(k),'XLabel'),'FontSize',FONT)	% 14-pt bold labels
