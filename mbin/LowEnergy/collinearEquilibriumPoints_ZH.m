function L = collinearEquilibriumPoints_ZH(prms)
...
dUdx   = @(x) x - (1-mu)*(x+mu)/abs(x+mu)^3 - mu*(x-1+mu)/abs(x-1+mu)^3 + dUdx_J2p(x) +   dUdx_J2s(x) +   dUdx_J4p(x) +   dUdx_J4s(x) +   dUdx_J6p(x) +   dUdx_J6s(x);
d2Udx2 = @(x) 1 + 2*(1-mu)/abs(x+mu)^3 + 2*mu/abs(x-1+mu)^3 +           d2Udx2_J2p(x) + d2Udx2_J2s(x) + d2Udx2_J4p(x) + d2Udx2_J4s(x) + d2Udx2_J6p(x) + d2Udx2_J6s(x);
...
for ii = 1:3
    switch points(ii)
        ...
        case 1      
            x_colVec(ii) = NewtonMethod(dUdx, d2Udx2, 1/2-mu);
            y_colVec(ii) = 0;
            
        % L2, right of p2   
        case 2
            x_colVec(ii) = NewtonMethod(dUdx, d2Udx2, (1-mu)+1e-5); 
            y_colVec(ii) = 0;
            
        % L3, left of p1
        case 3
            x_colVec(ii) = NewtonMethod(dUdx, d2Udx2, -(1-mu)); 
...
if abs((L(1,1) - L(2,1))) < 1e-11
    warning('L1 and L2 might be the same solution')
end
end
