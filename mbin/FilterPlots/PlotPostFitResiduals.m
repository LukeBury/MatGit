...
function PlotPostFitResiduals(time, eis, statNums, sig1, sig2, fs)
...
for k = 1:size(statPrs,1)-1
    ...
    t2 = statPrs(k+1,2);
    if statPrs(k+1,1) == 1
        p1 = plot(time(t1:t2),eis(1,t1:t2),'mx');
        s1_On = 1;
    elseif statPrs(k+1,1) == 2
        p2 = plot(time(t1:t2),eis(1,t1:t2),'rx');
        s2_On = 1;
    elseif statPrs(k+1,1) == 3
        p3 = plot(time(t1:t2),eis(1,t1:t2),'bx');
        s3_On = 1;
    end
end
...
for k = 1:size(statPrs,1)-1
    ...
    t2 = statPrs(k+1,2);
    if statPrs(k+1,1) == 1
        p1 = plot(time(t1:t2),eis(2,t1:t2),'mx');
    elseif statPrs(k+1,1) == 2
        p2 = plot(time(t1:t2),eis(2,t1:t2),'rx');
    elseif statPrs(k+1,1) == 3
        p3 = plot(time(t1:t2),eis(2,t1:t2),'bx');
    end
end
