function [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_zd0Fixed(PO_guess, N_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed zd0
%       
% ------------------------------------------------------------------------
%%% Inputs
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_zdFixed(N_Nodes, PO_guess(6), F_new, integrator, integrator_options, prms);
...
elseif constraint_error <= constraint_error_tol
    PO_new = [F_new(1:5)', PO_guess(6), F_new(end)]';
end  
