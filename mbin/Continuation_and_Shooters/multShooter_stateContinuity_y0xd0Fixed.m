function [DF_mat, constraints_vec] = multShooter_stateContinuity_y0xd0Fixed(n_Nodes, F, dynamicsWithSTM, options, prms, y0, xd0)
%%% Description
%       Multiple shooter for state continuity of a 6-d state (ex, [x; y; z;
%       xd; yd; zd]) with y0 and xd0 of the first node fixed
%       
% ------------------------------------------------------------------------
%%% Inputs
...
parfor node_i = 1:n_Nodes
    ...
    %%% Integrate node            
    if node_i == 1
        [~, X_node] = ode113(dynamicsWithSTM, [0, F(end)/n_Nodes], [F(1); y0; F(2); xd0; F(3:4); stm0_colVec], options, prms);
    else
    ...
    elseif node_i == n_Nodes % If the last node
        ...
        %%% of first node
        constraints_vec_temp((6*(node_i-1)+1):(6*(node_i-1)+6)) = X_node(end,1:6)' - [F(1); y0; F(2); xd0; F(3:4)];
