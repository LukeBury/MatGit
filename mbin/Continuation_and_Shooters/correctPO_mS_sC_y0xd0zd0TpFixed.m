function [PO_new, counter, constraint_error] = correctPO_mS_sC_y0xd0zd0TpFixed(PO_guess, n_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize, y0, xd0, zd0, Tp)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed time period and y0, xd0, and zd0 of first node
%       
% ------------------------------------------------------------------------
%%% Inputs
...
% zd0                  - [scalar] Fixed zd0 of first node
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_y0xd0zd0TpFixed(n_Nodes, F_new, integrator, integrator_options, prms, y0, xd0, zd0, Tp);
...
elseif constraint_error <= constraint_error_tol
    PO_new = [F_new(1), y0, F_new(2), xd0, F_new(3), zd0, Tp]';
end  
