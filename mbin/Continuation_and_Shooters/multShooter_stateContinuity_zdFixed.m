function [DF_mat, constraints_vec] = multShooter_stateContinuity_zdFixed(n_Nodes, zd0, F, dynamicsWithSTM, options, prms)
%%% Description
%       Multiple shooter for state continuity of a 6-d state (ex, [x; y; z;
%       xd; yd; zd]). Initial zd-guess is fixed and won't change
%       
% ------------------------------------------------------------------------
%%% Inputs
...
parfor node_i = 1:n_Nodes
    ...
    %%% Integrate node
    if node_i == 1
        [~, X_node] = ode113(dynamicsWithSTM, [0, F(end)/n_Nodes], [F(1:5); zd0; stm0_colVec], options, prms);
    else
    ...
    elseif node_i == n_Nodes % If the last node
        ...
        %%% of first node
        constraints_vec_temp((6*(node_i-1)+1):(6*(node_i-1)+6)) = X_node(end,1:6)' - [F(1:5); zd0];

        %%% Add Identity to the DF matrix
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),1:5) = -[eye(5); zeros(1,5)];
    ...
    if node_i == 1
        DF_mat_temp(1:6, 1:5) = stm_tf_t0(:,1:5);
    else
