function [DF_mat, constraints_vec] = multShooter_stateContinuity_zTpFixed(n_Nodes, F, dynamicsWithSTM, options, prms, Tp_des, z_des)
%%% Description
...
[~, X1_node] = ode113(dynamicsWithSTM, [0, Tp_des/n_Nodes], [F(1:2); z_des; F(3:5); stm0_colVec], options, prms);
...
parfor node_i = 1:n_Nodes
    ...
    if node_i == 1
        ...
        [~, X_node] = ode113(dynamicsWithSTM, [0, Tp_des/n_Nodes], [F((6*(node_i-1)):(6*(node_i-1)+5)); stm0_colVec], options, prms);
    end
    ...
    if node_i < n_Nodes % If not the last node
        ...
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i)):(6*(node_i)+5)) = -eye(6);
