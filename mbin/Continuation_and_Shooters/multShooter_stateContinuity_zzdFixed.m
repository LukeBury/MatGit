function [DF_mat, constraints_vec] = multShooter_stateContinuity_zzdFixed(n_Nodes, z0, zd0, F, dynamicsWithSTM, options, prms)
%%% Description
%       Multiple shooter for state continuity of a 6-d state (ex, [x; y; z;
%       xd; yd; zd]). Initial z and zd are fixed and won't change
%       
% ------------------------------------------------------------------------
%%% Inputs
%       n_Nodes         - [1x1] Number of nodes in multiple shooter 
%       zd0             - [1x1] Desired zd value
...
%     %%% Integrate node
%     if node_i == 1
%         [~, X1_node] = ode113(dynamicsWithSTM, [0, F(end)/n_Nodes], [F(1:2); z0; F(3:4); zd0; stm0_colVec], options, prms);
%         X_node = X1_node;
%     else
...
%         %%% state of next node
%         X_next0 = F((node_i*6-1):(node_i*6+4));
%         constraints_vec((6*(node_i-1)+1):(6*(node_i-1)+6)) = X_node(end,1:6)' - X_next0;
% 
%         %%% Add Identity to the DF matrix
%         DF_mat((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i-1)+5):(6*(node_i-1)+10)) = -eye(6);
%         
...
%         %%% Add Identity to the DF matrix
%         tempMat = -eye(6);
%         DF_mat((6*(node_i-1)+1):(6*(node_i-1)+6),1:4) = tempMat(:,[1,2,4,5]);
% 
%     end
% 
...
%     if node_i == 1
% %         DF_mat(1:6, 1:5) = stm_tf_t0(:,1:5);
%         DF_mat(1:6, 1:4) = stm_tf_t0(:,[1:2,4:5]);
%     else
%         DF_mat((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i-1)-1):(6*(node_i-1)+4)) = stm_tf_t0;
%     end
% 
...
parfor node_i = 1:n_Nodes
    ...
    elseif node_i == n_Nodes % If the last node
        ...
        %%% Add Identity to the DF matrix
        tempMat = -eye(6);
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),1:4) = tempMat(:,[1,2,4,5]);
    ...
    stm_tf_t0 = reshape(X_node(end,7:42),6,6);
    if node_i == 1
        DF_mat_temp(1:6, 1:4) = stm_tf_t0(:,[1:2,4:5]);
    else
