...
% -------------------------------------------------
%%% Choose number of steps taken to get from mu_1 to mu_2
% n_steps = 100; 989 
% n_steps = 200; 989
n_steps = 300; 
% n_steps = 500; 989
...
%     plot_PO_indices = 80; 989 % Overriding 
...
%%% Set up mass ratios to continue over
mu_1 = secondary.MR;
mu_2 = bodies.ganymede.MR;
% mu_2 = bodies.enceladus.MR;
% mu_2 = bodies.moon.MR;

mu_vec = logspace(log10(mu_1), log10(mu_2), n_steps);
...
for step_i = 1:n_steps
    prms.u = mu_vec(step_i);

    %%% If we're predicting the Tp
    if shooter_y0TpFixed || shooter_y0xd0TpFixed || shooter_y0xd0zd0TpFixed
        ...
        PO_guess(7) = PO_guess(7)*scale_Tp;
    ...
    elseif shooter_y0TpFixed
        [PO_result, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0TpFixed(PO_guess, n_Nodes, @Int_CR3BnSTM, options, prms, error_tol, ms_iterMax, stepSize, PO_guess(2), PO_guess(7));
    elseif shooter_y0xd0TpFixed
        [PO_result, counter, constraint_error] = correctPO_mS_sC_y0xd0TpFixed(PO_guess, n_Nodes, @Int_CR3BnSTM, options, prms, error_tol, ms_iterMax, stepSize, PO_guess(2), PO_guess(4), PO_guess(7));
    elseif shooter_y0xd0zd0TpFixed
        [PO_result, counter, constraint_error] = correctPO_mS_sC_y0xd0zd0TpFixed(PO_guess, n_Nodes, @Int_CR3BnSTM, options, prms, error_tol, ms_iterMax, stepSize, PO_guess(2), PO_guess(4), PO_guess(6), PO_guess(7));
    end
    warning('on')
    
    %%% Calculate propagation error
    [T_result, X_result] = ode113(@Int_CR3BnSTM, [0, PO_result(7)], [PO_result(1:6); stm0_colVec], options, prms);
