function [DF_mat, constraints_vec] = multShooter_stateContinuity_y0xd0zd0TpFixed(n_Nodes, F, dynamicsWithSTM, options, prms, y0, xd0, zd0, Tp_des)
%%% Description
...
DF_mat          = zeros(6*n_Nodes, 6*n_Nodes-3);
...
parfor node_i = 1:n_Nodes
    ...
    DF_mat_temp          = zeros(6*n_Nodes, 6*n_Nodes-3);

    %%% Integrate node            
    if node_i == 1
        [~, X_node] = ode113(dynamicsWithSTM, [0, Tp_des/n_Nodes], [F(1); y0; F(2); xd0; F(3); zd0; stm0_colVec], options, prms);
    else
        [~, X_node] = ode113(dynamicsWithSTM, [0, Tp_des/n_Nodes], [F((6*(node_i-1)-2):(6*(node_i-1)+3)); stm0_colVec], options, prms);
    end
