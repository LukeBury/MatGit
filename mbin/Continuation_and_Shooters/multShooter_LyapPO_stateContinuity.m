function [DF_mat, constraints_vec] = multShooter_LyapPO_stateContinuity(n_Nodes, x0, F, dynamicsWithSTM, options, prms)
%%% Description
%       Multiple shooter for state continuity of a 6-d state (ex, [x; y; z;
%       xd; yd; zd]). For use specifically with finding Lyapunovs based on
%       an initial x-axis guess that doesn't change
%       
% ------------------------------------------------------------------------
%%% Inputs
...
parfor node_i = 1:n_Nodes
    ...
    DF_mat_temp          = zeros(6*n_Nodes,6*(n_Nodes-1)+2);
    %%% Integrate node   
    if node_i == 1
        X_node = X1_node;
    else
    ...
    if node_i < n_Nodes % If not the last node
        %%% Differencing end-state of current node with beginning
        %%% state of next node
        constraints_vec_temp((6*(node_i-1)+1):(6*(node_i-1)+6)) = X_node(end,1:6)' - F(((node_i-1)*6+2):((node_i-1)*6+7));

        %%% Add Identity to the DF matrix
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i-1)+2):(6*(node_i-1)+7)) = -eye(6);
    ...
    elseif node_i == n_Nodes % If the last node
        ...
        %%% Add Identity to the DF matrix
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),1) = [0; 0; 0; 0; -1; 0];
    ...
    stm_tf_t0 = reshape(X_node(end,7:42),6,6);
    if node_i == 1
        DF_mat_temp(1:6,1) = stm_tf_t0(:,5);
    else
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i-2)+2):(6*(node_i-2)+7)) = stm_tf_t0;
    end

    %%% Add end-state dynamics to the DF matrix
    endstateDynamics = dynamicsWithSTM(0,[X_node(end,1:6)'; zeros(36,1)], prms);
    DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(n_Nodes-1)+2)) = endstateDynamics(1:6);
