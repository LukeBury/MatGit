function [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0Fixed(PO_guess, n_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize, y0)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed y0
%       
% ------------------------------------------------------------------------
%%% Inputs
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_y0Fixed(n_Nodes, F_new, integrator, integrator_options, prms, y0);
