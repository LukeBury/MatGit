function [DF_mat, constraints_vec] = multShooter_stateContinuity_y0xd0zd0Fixed(n_Nodes, F, dynamicsWithSTM, options, prms, y0, xd0, zd0)
%%% Description
%       Multiple shooter for state continuity of a 6-d state (ex, [x; y; z;
%       xd; yd; zd]) with y0, xd0, and zd0 of the first node fixed
%       
% ------------------------------------------------------------------------
%%% Inputs
...
%       xd0             - [scalar] Value for xd0 of first node
%       zd0             - [scalar] Value for zd0 of first node
% ------------------------------------------------------------------------
%%% Outputs
...
parfor node_i = 1:n_Nodes
    ...
    %%% Integrate node            
    if node_i == 1
        [~, X_node] = ode113(dynamicsWithSTM, [0, F(end)/n_Nodes], [F(1); y0; F(2); xd0; F(3); zd0; stm0_colVec], options, prms);
    else
        [~, X_node] = ode113(dynamicsWithSTM, [0, F(end)/n_Nodes], [F((6*(node_i-1)-2):(6*(node_i-1)+3)); stm0_colVec], options, prms);
    end
    ...
    if node_i < n_Nodes % If not the last node
        ...
        %%% state of next node
        constraints_vec_temp((6*(node_i-1)+1):(6*(node_i-1)+6)) = X_node(end,1:6)' - F((node_i*6-2):(node_i*6+3));

        %%% Add Identity to the temporary DF matrix
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i-1)+4):(6*(node_i-1)+9)) = -eye(6);

    elseif node_i == n_Nodes % If the last node
        %%% Differencing end-state of final node with beginning
        %%% of first node
        constraints_vec_temp((6*(node_i-1)+1):(6*(node_i-1)+6)) = X_node(end,1:6)' - [F(1); y0; F(2); xd0; F(3); zd0];

        %%% Add Identity to the temporary DF matrix
        neye6 = -eye(6);
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),1:3) = neye6(:,[1,3,5]);
    end
    ...
    if node_i == 1
        DF_mat_temp(1:6, 1:3) = stm_tf_t0(:,[1,3,5]);
    else
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*(node_i-1)-2):(6*(node_i-1)+3)) = stm_tf_t0;
    end

    %%% Add end-state dynamics to the temporary DF matrix
    endstateDynamics = dynamicsWithSTM(0,[X_node(end,1:6)'; zeros(36,1)], prms);
    DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),(6*n_Nodes-2)) = endstateDynamics(1:6);
