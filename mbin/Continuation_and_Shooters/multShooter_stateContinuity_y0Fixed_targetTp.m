function [DF_mat, constraints_vec] = multShooter_stateContinuity_y0Fixed_targetTp(n_Nodes, F, dynamicsWithSTM, options, prms, y0, Tdes)
%%% Description
...
parfor node_i = 1:n_Nodes
    ...
    elseif node_i == n_Nodes % If the last node
        ...
        %%% Add Identity to the temporary DF matrix
        neye6 = -eye(6);
        DF_mat_temp((6*(node_i-1)+1):(6*(node_i-1)+6),1:5) = [neye6(1:6,1), neye6(1:6,3:6)];
    end
    ...
    if node_i == 1
        ...
        DF_mat_temp(1:6, 2:5) = stm_tf_t0(:,3:6);
    else
    ...
    %%% Add Tp/Tp derivative to DF and Tdes-Tp to constraints
    constraints_vec_temp(end) = Tdes - F(end);
    DF_mat_temp(6*n_Nodes+1, 6*n_Nodes) = -1;
