function [F_new, constraint_error, DF_mat, iteration_counter] = pseudoArclengthContinuation_multShoot_stateContinuity_y0Fixed(error_tol, iterMax, n_Nodes, F_new, dynamicsWithSTM, options, prms, familyIndex, stepSize, nullVecDF, ds_PO, y0)
%%% Description
...
%       y0              - [1x1] Value for y0 of first node. probably 0
% ------------------------------------------------------------------------
%%% Outputs
...
%%% This section of commented-out code was an attempt to address the
%%% problem of an F_new leading to a DF_mat with an empty null space.
% % % % % -------------------------------------------------
% % % % %%% Pre-While-Loop work
% % % % % -------------------------------------------------
...
% % % % % emptyNullCounter = 0;
% % % % DF_mat = 0;
% % % % while (constraint_error > error_tol) || (isempty(null(DF_mat)))
% % % %     
...
% % % %     % populate constraint vector and DF matrix
% % % %     % --------------------------
% % % %     [DF_mat, constraints] = multShooter_stateContinuity_y0Fixed(n_Nodes, F_new, dynamicsWithSTM, options, prms, y0);
% % % %     989;
% % % %     
% % % %     % --------------------------
...
% % % %         if (constraint_error > error_tol) || (isempty(null(DF_mat)))
...
while ((constraint_error > error_tol) && (iteration_counter < iterMax)) 
    ...
    if isempty(null(DF_mat))
%         familyIndex
%         iteration_counter
%         
%         989;
        
%         if emptyNullCounter >=5
%             warning('DF_mat of current F_new has no null space')
%             break
%         end
%         F_new = F_old;
%         ds_PO = ds_PO * .95;
%         emptyNullCounter = emptyNullCounter + 1;
%         continue
    end
    % --------------------------
    % If familyIndex == 1, do a simplified version of things to find next
...
%             F_new = F_new - DH'*((DH*(DH'))\constraints_aug); %Form for generalized inverse
        end
