function [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_TpFixed(PO_guess, n_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize, Tp)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed time period
%       
% ------------------------------------------------------------------------
%%% Inputs
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_TpFixed(n_Nodes, F_new, integrator, integrator_options, prms, Tp);
