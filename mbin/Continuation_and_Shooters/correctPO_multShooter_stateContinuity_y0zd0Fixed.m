function [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0zd0Fixed(PO_guess, n_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize, y0, zd0)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed y0 and zd0
%       
% ------------------------------------------------------------------------
%%% Inputs
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_y0zd0Fixed(n_Nodes, F_new, integrator, integrator_options, prms, y0, zd0);
...
elseif constraint_error <= constraint_error_tol
    PO_new = [F_new(1); y0; F_new(2:4); zd0; F_new(end)];
end  
