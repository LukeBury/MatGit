function [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_x0y0Fixed(PO_guess, n_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize, x0, y0)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed x0 and y0
%       
% ------------------------------------------------------------------------
%%% Inputs
...
% x0                   - [scalar] fixed x0 value of first node
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_x0y0Fixed(n_Nodes, F_new, integrator, integrator_options, prms, x0, y0);
...
elseif constraint_error <= constraint_error_tol
    PO_new = [x0, y0, F_new(1:4)', F_new(end)]';
end  
