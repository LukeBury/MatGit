function [PO_new, counter, constraint_error] = correctPO_multShooter_stateContinuity_y0xd0zd0Fixed(PO_guess, n_Nodes, integrator, integrator_options, prms, constraint_error_tol, ms_iterMax, stepSize, y0, xd0, zd0)
%%% Description
% Multiple shooting algorithm to correct a periodic orbit for state
% continuity with a fixed y0, xd0, and zd0
%       
% ------------------------------------------------------------------------
%%% Inputs
...
% xd0                  - [scalar] fixed xd0 value of first node
% zd0                  - [scalar] fixed zd0 value of first node
% ------------------------------------------------------------------------
%%% Outputs
...
while (constraint_error > constraint_error_tol) && (counter < ms_iterMax)
    ...
    % --------------------------
    [DF_mat, constraints] = multShooter_stateContinuity_y0xd0zd0Fixed(n_Nodes, F_new, integrator, integrator_options, prms, y0, xd0, zd0);
...
elseif constraint_error <= constraint_error_tol
    PO_new = [F_new(1); y0; F_new(2); xd0; F_new(3); zd0; F_new(end)];
end  
