function L = collinearEquilibriumPoints_J2pJ4pJ6pJ2s_scaled(prms)
...
dUdx   = @(x) prms.n*prms.n*x - (1-mu)*(x+mu)/abs(x+mu)^3 - mu*(x-1+mu)/abs(x-1+mu)^3 + (dUdx_J2p(x)   + dUdx_J2s(x)   + dUdx_J4p(x)   + dUdx_J6p(x))*prms.pertScale;
d2Udx2 = @(x) prms.n*prms.n + 2*(1-mu)/abs(x+mu)^3 + 2*mu/abs(x-1+mu)^3 +               (d2Udx2_J2p(x) + d2Udx2_J2s(x) + d2Udx2_J4p(x) + d2Udx2_J6p(x))*prms.pertScale;
