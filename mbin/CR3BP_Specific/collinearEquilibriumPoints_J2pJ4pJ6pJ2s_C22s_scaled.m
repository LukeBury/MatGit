function L = collinearEquilibriumPoints_J2pJ4pJ6pJ2s_C22s_scaled(prms)
...
dUdx   = @(x) prms.n*prms.n*x - (1-mu)*(x+mu)/abs(x+mu)^3 - mu*(x-1+mu)/abs(x-1+mu)^3 + (dUdx_J2p(x)   + dUdx_J2s(x)   + dUdx_J4p(x)   + dUdx_J6p(x)   + dUdx_C22s(x)  )*prms.pertScale;
d2Udx2 = @(x) prms.n*prms.n + 2*(1-mu)/abs(x+mu)^3 + 2*mu/abs(x-1+mu)^3 +               (d2Udx2_J2p(x) + d2Udx2_J2s(x) + d2Udx2_J4p(x) + d2Udx2_J6p(x) + d2Udx2_C22s(x))*prms.pertScale;
...
for ii = 1:3
    switch points(ii)
        ...
        case 1      
            x_colVec(ii) = NewtonMethod(dUdx, d2Udx2, 1/2-mu, 1e-15, 1000);
            y_colVec(ii) = 0;
            
        % L2, right of p2   
        case 2
            x_colVec(ii) = NewtonMethod(dUdx, d2Udx2, (1-mu)+1e-5, 1e-15, 1000); 
            y_colVec(ii) = 0;
            
        % L3, left of p1
        case 3
            x_colVec(ii) = NewtonMethod(dUdx, d2Udx2, -(1-mu), 1e-15, 1000); 
